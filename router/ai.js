...
// Load HUMAN-VERIFIED dieline examples (few-shot). Prompt-only rules hit a noise
// floor on the tuck-family (1/2/3/4/11) distinction, so instead we show the model
// real dielines a person has already labelled with the CORRECT template + the reason.
// LABELLING BY FOLDER: labeled_dielines/<1..12>/<image>.{jpg,png} — the folder NAME is
// the correct template. Optional sibling <image>.txt = the reason/what to notice. The
// team just drops a verified dieline into the right numbered folder; no code/JSON edit.
// The store grows → accuracy improves. NOTE: few-shot over-generalises when the set is
// tiny/unbalanced (one RTE example drags every tuck box to 1); keep it OFF
// (AI_FEWSHOT_DIELINES=1 to enable) until the confusable templates each have several.
const LABELED_EXAMPLES = []
try {
	const ldir = path.join(__dirname, 'labeled_dielines')
	for (let id = 1; id <= 12; id++) {
		const tdir = path.join(ldir, String(id))
		if (!fs.existsSync(tdir)) continue
		for (const fname of fs.readdirSync(tdir)) {
			if (!/\.(jpe?g|png|webp)$/i.test(fname)) continue
			const p = path.join(tdir, fname)
			const txtPath = p.replace(/\.[^.]+$/, '.txt')
			const why = fs.existsSync(txtPath) ? fs.readFileSync(txtPath, 'utf8').trim() : ''
			LABELED_EXAMPLES.push({
				template: id,
				why,
				b64: fs.readFileSync(p).toString('base64'),
				media: /\.png$/i.test(fname) ? 'image/png' : (/\.webp$/i.test(fname) ? 'image/webp' : 'image/jpeg'),
			})
		}
	}
	const byT = LABELED_EXAMPLES.reduce((m, e) => { m[e.template] = (m[e.template] || 0) + 1; return m }, {})
	console.log('AI: loaded ' + LABELED_EXAMPLES.length + ' labeled dieline examples (few-shot) by template ' + JSON.stringify(byT))
} catch (e) {
	console.warn('AI: failed to load labeled dielines:', e.message)
}
...

{
  ...
  "use_previous_plate": "true ONLY when the doc EXPLICITLY says to reuse the old plate (ใช้ Plate เก่า / ใช้เพลทเดิม / เพลทเดิม / ใช้แม่พิมพ์เดิม / reuse old plate). A 'Rep.'/'Reprint' title alone is NOT enough — a reprint defaults to NEW plates (false) unless the spec explicitly says to reuse them. Do NOT infer plate reuse from the reprint status.",
  ...
  "components": [
    {
      ...
      "glued_spots": "number — ONLY set this when the TEXT of the spec EXPLICITLY says how many glue spots ('ติดกาว 2 จุด', 'ปะกาว 3 จุด', 'ไดคัทติดกาว 6 จุด', 'ค่าติดกาว N จุด', 'ติดลิ้นกาว N จุด', 'ติดกาวลิ้น N จุด', 'ปะลิ้นกาว N จุด', 'glue 3 points'). ALWAYS capture the number even when it is 1 — 'ติดกาว 1 จุด' → glued_spots=1, 'ติดลิ้นกาว 1 จุด' → glued_spots=1, 'ค่าติดกาว 1 จุด' → glued_spots=1. NEVER guess or infer from a dieline image. If the spec MENTIONS gluing/assembly ('ติดกาว / ปะกาว / ค่าติดกาว / ติดลิ้นกาว / ปะลิ้นกาว / gluing / assembly') but WITHOUT a number → set glued_spots=1 (a bare 'ติดกาว' = the single side-seam glue = 1 spot). ONLY if the spec does NOT mention gluing at all → OMIT this field entirely (do NOT force a default when there is no glue text). The glue-spot number alone is NOT enough evidence to pick a particular box_template_id — let the visual match against the 12 template images decide the template; glued_spots is just a sub-field that fills the form's 'N จุด' input.",
...
- MULTI-DESIGN JOBS (multi-F): If the doc lists multiple DIFFERENT design codes / SKUs that each have their own qty (e.g. "งานมี 8 แบบ", "design 02 = 10000 pcs, design 03 = 25000 pcs", "Wallet 2000 / Sticker 500 / ..."):
    ...
    * EXCEPTION — SHARED QTY TIERS: if the designs do NOT each carry their own distinct qty but instead SHARE a common list of quantity TIERS (e.g. "งานมี 12 แบบ ... ราคาต่อแบบ ... qty 3000 5000 7000 10000 20000" — all 12 designs priced across the SAME 5 tiers), THEN put the shared tiers in quantities[] (quantities=[3000,5000,7000,10000,20000]) AND still set is_multiple_f=true with f_codes for the designs. Leave quantities EMPTY ONLY when each f_code carries its OWN separate qty.
- SAME DESIGN, MULTIPLE PRINT PASSES (multi-F, ONE component — CRITICAL): If the PRINT section lists the SAME F-code / design printed in several PASSES / RUNS — e.g. "F016062-พิมพ์ครั้งที่ 1 : 5/0", "F016062 พิมพ์ครั้งที่ 2 : 5/0", "พิมพ์ 2 รอบ", "print pass 1/2", "รอบที่ 1/2", "พิมพ์ 2 ครั้ง" — this is ONE physical box (ONE component with ONE PAPER + ONE box type) printed in multiple runs, NOT multiple components. The SAME base code appearing on 2+ PRINT lines is the tell. RULE:
    * Set "is_multiple_f": true
    * Emit EXACTLY ONE component (the single box). NEVER create a separate component per print pass — that is WRONG (it produces N components each with N F).
    * Put ONE entry per PASS in "f_codes" (e.g. code "F016062-1", "F016062-2", or the pass label). The colors (e.g. 5/0) belong to each pass's f_code.
    * Each pass runs the SAME total qty (same sheets printed again) — do NOT split the total across passes.
    * Contrast: DIFFERENT base codes / SKUs = multiple designs (rule above); SAME base code across passes = this rule.
...
- KRAFTWRAP / PER-PACK COUNT: "kraftwrap N pcs/pack", "N pcs/pack", "บรรจุ N ชิ้น/แพค", "N ชิ้น/แพค", "N ชิ้น/pack" → put N in packing.shrink_per_unit (the per-pack piece count). Always capture it whenever a per-pack piece count is stated.
...
- "color_outside / color_inside" = TOTAL number of printing colors per side. COUNT GENEROUSLY — include CMYK + every additional ink/varnish layer applied via the press:
    ...
    * "Spot UV" / "UV ดิฟออฟ" / "UV ดรอปออฟ" / "UV Drip Off" when applied via PRINTING (and also adds a coating row — see coatings below) → +1 — BUT ONLY when there is NO explicit 'N/M สี'/'N/M Colors' total; an explicit count like '2/0 Colors' already excludes it (see EXPLICIT COUNT IS FINAL above)
    ...
    * If the colour count is given as a RANGE (e.g. 'COLOR Q'TY: 3-5', '4-6 สี', '1-8') → use the MAXIMUM of the range (conservative for estimate pricing). '3-5' → color_outside=5.\n    * EXPLICIT COUNT IS FINAL: when the spec states 'N/M สี' or 'N/M Colors', use N and M EXACTLY — do NOT add ANYTHING to that number. Coating/finishing layers ('ขัดเงา', 'ขัดมัน', 'เคลือบเงา', 'Gloss'/'Matt' varnish, 'UV Drip off', 'Spot UV', 'เคลือบ ...') go to coatings[] ONLY, NEVER to the color count — even when they are 'applied via printing'. The generous +1 rule below applies ONLY when colors are described QUALITATIVELY with NO explicit N/M total (e.g. 'CMYK + ขาวตีพื้น'). Examples: '4/0 สี (P186,P199,P269,P3272 + ขัดเงา)' → 4 ; '2/0 Colors เคลือบ UV Drip off' → 2 ; '5/0 Colors (CMYK+PMS)' → 5.
...
- For box_template_id: see the box_template_id field description in the schema above for the full 12-template map. Quick keyword → template reference:
    ...
    * GLUE-SPOT COUNT is a HINT, NEVER a template decider on its own: "ติดกาว / ปะกาว / ค่าติดกาว 3 จุด" — 3+ glue spots (glued_spots ≥ 3) — SUGGESTS an auto-bottom lock (side seam + 2 bottom tabs = Template 4 TTAB). BUT a glue-spot COUNT BY ITSELF does NOT pick a template — the visual dieline match and explicit STYLE WORDS always decide, and the POLICY still holds: NO style word AND no dieline → Template 12 (Custom). Pick Template 4 from glue spots ONLY when the text ALSO has an auto-bottom / bottom-glue STYLE WORD ("ก้นกล่อง", "ทากาวก้น", "auto-bottom", "TTAB", "crash bottom") OR a dieline image shows auto-bottom construction. If there is NO box-style word AND no dieline, a 3-dimension box with "ติดกาว N จุด" (even ≥3) → Template 12 (Custom) + add box_template_id to _uncertain — do NOT force-fit Template 4 from the count alone (the count still fills the glued_spots field). glued_spots = 1 → side seam only. Corrugated is the same: a glue count alone does NOT make it Template 4 — it still needs the style word or dieline.
...
- Common box_template_id MISTAKES to avoid:
    ...
    * 3+ glue spots on its own → NOT a strong template signal. The form's glued_spots field is independent of box_template_id. Most folding cartons have only 1 side-glue tab; a high count HINTS at auto-bottom (Template 4) — but Template 4 still requires the text to say bottom glue OR a dieline. Without bottom-glue text: DEFER to the box_template_id rules — a tuck-TOP style word present → Template 3; NO box-style word AND no dieline → Template 12 (Custom) + _uncertain. Never force Template 3 (or 4) from a glue count alone.
...
- OUTER SHAPE → CUSTOM (the KEY Custom rule for dielines — look CAREFULLY at the outermost outline): output a component field "outer_shape" and classify the OUTERMOST OUTLINE of the whole flat dieline as EXACTLY ONE of these strings:
    * "plain_rectangle" — a clean rectangular envelope: a plain tuck / seal-end / tray / sleeve whose panels and flaps all sit inside a rectangle. (A mirrored side-glue seam is STILL plain_rectangle — mirror does not matter.)
...
- FLAT / OPEN SIZE (do this for EVERY dieline — uploaded dielines are all treated as Custom, so the form needs the FLAT-SHEET size, NOT the folded box W/L/H): output open_size_mm = {width, length} = the OVERALL OUTERMOST dimensions of the whole flat. READ IT from the outermost DIMENSION-LINE ARROWS that span the entire flat — e.g. a long arrow "392.5" across the top and "259" down the right side → open_size_mm {width:259, length:392.5} — OR from a title "Size A×B". Use ONLY the TOTAL outer numbers; NEVER a single panel's number (e.g. the small 30 / 160 panel widths) and never the folded box size. Still also output dimensions_mm (the folded box W/L/H) for reference. EXCEPTION: if the drawing is a LAYOUT / N-up (see the N-UP rule below), OMIT open_size_mm. If no overall flat dimension can be read confidently, OMIT open_size_mm — the user will enter it — rather than guess a panel number.
...
- DIELINE TITLE/HEADER SIZE (READ IT EVERY TIME): dieline drawings almost always print the size in a TITLE/HEADER line next to a job code, e.g. "F009898 INNER BOX NEW R2 Size 16.25'' X 35.5'' = 2" or "LAY F011314 - กล่อง MAC Size 18\"x 25\" = 4" or "Size 37x20 cm". This header is the MOST RELIABLE size on the drawing — ALWAYS find and read it (do not stop at the ruler/dimension lines), even when it sits amid a job code, a "LAY" prefix, or Thai text. Capture it VERBATIM (with unit and any "= N") in stated_flat_size. Inches may be written ", '', or "in".
- DIELINE LAYOUT / N-UP (do NOT feed a sheet size into open_size_mm): if the header shows "= N" with N≥2, OR starts with "LAY" / "LAYOUT" / "การวาง", the labelled size is the WHOLE IMPOSED SHEET holding N boxes — NOT one box. In that case: (a) keep the verbatim sheet size in stated_flat_size (e.g. "18x25 in, LAY = 4-up"); (b) OMIT open_size_mm — do NOT output the sheet size as the box's flat size (it is ~N× too large, and a wrong single-box size corrupts the price); (c) add "open_size_mm" to _uncertain and add a "notes" line that states the LAYOUT size and N-up count VERBATIM so the estimator can work out the single box — e.g. "ไดไลน์เป็น LAYOUT: 18x25 in = 4-up (ขนาดในรูปคือทั้งแผ่น N กล่อง) — กรุณากรอก Open Size กล่องเดียว". ONLY when the drawing shows a SINGLE box (no "= N", no "LAY") may you put the read size into open_size_mm.
...
- REPRINT DETECTION (is_reprinted): set is_reprinted=1 ONLY when there is a clear signal that this job is a RE-RUN of a previous job. Valid signals: (a) title starts with "Rep." / "Reprint" / "REP.", (b) the doc explicitly says พิมพ์ซ้ำ / งานซ้ำ / งานเก่า / reprint, or (c) the design/job code carries a revision suffix like "R1" / "R2" (e.g. "PGIB13007R1"). If none of these are present, set is_reprinted=0 — do NOT infer reprint from anything else.
- USE_PREVIOUS_PLATE IS A SEPARATE DECISION — it is NOT implied by is_reprinted. A reprint does NOT automatically reuse the old plate; most reprints still make new plates. Set use_previous_plate=true ONLY when the doc EXPLICITLY states plate reuse: "ใช้ Plate เก่า" / "ใช้เพลทเดิม" / "เพลทเดิม" / "ใช้แม่พิมพ์เดิม" / "reuse old plate". If the doc says nothing about plates, set use_previous_plate=false (assume NEW plates) — EVEN for a "Rep." / reprint job. Never turn it on just because the title starts with "Rep.".
...
OUTPUT: {"job_name":"กล่อง Inner box","customer_name":"ABC Corp","print_type":"Offset","quantities":[5000,10000],"components":[{"type":1,"box_template_id":12,"dimensions_mm":{"width":80,"length":120,"height":40},"paper_type":"A/C","paper_gram":350,"color_outside":4,"color_inside":0,"coatings":[{"option":"Gloss","type":"OPP 1 s"}]}],"deliveries":[{"destination":"กรุงเทพ"}],"_uncertain":["box_template_id"]}
(note: 'Inner box' has NO explicit style word and no dieline → Template 12 Custom + flag box_template_id uncertain, per rule 4/5/8. It would be a REAL template only if the spec said e.g. 'RTE' / 'sleeve' / 'tray' / 'gable', or a dieline image was provided.)
...
async function buildContentFromUpload(textInput, files) {
	...
	// Human-verified examples (few-shot): real dielines already labelled with the
	// CORRECT template + reasoning. These are GROUND TRUTH — they outrank the
	// idealised template reference images when the uploaded dieline matches one.
	// Placed last among the constant blocks so they + everything above stay cached.
	//
	// GATED OFF until the store is diverse enough. Testing showed that with only a
	// FEW examples the model over-generalises — it matches on brand/silhouette, not
	// construction, and drags unrelated boxes to the example's template (an Applaws
	// INNER carton followed the Applaws DISPLAY example to Custom; a plain TTAB
	// followed the reverse-tuck example to Template 1/2). Few-shot only helps once
	// the store has several DIVERSE, verified cases. Enable with AI_FEWSHOT_DIELINES=1
	// (and re-measure on held-out files) after the team fills labeled_dielines.json.
	if (LABELED_EXAMPLES.length > 0 && process.env.AI_FEWSHOT_DIELINES === '1') {
		content.push({ type: 'text', text: 'VERIFIED EXAMPLES — the following are REAL dielines that a human expert has already checked and labelled with the CORRECT template number. Treat these as ground truth: if the uploaded dieline matches the construction of one of them, use that verified template. Study the reasoning so you apply the same logic to similar boxes:' })
		LABELED_EXAMPLES.forEach((ex, idx) => {
			content.push({ type: 'text', text: 'VERIFIED → Template #' + ex.template + '. Why: ' + ex.why })
			const block = {
				type: 'image',
				source: { type: 'base64', media_type: ex.media, data: ex.b64 },
			}
			if (idx === LABELED_EXAMPLES.length - 1) {
				block.cache_control = { type: 'ephemeral' }
			}
			content.push(block)
		})
