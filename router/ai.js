...
6. dimensions_mm:
   • LENGTH vs WIDTH — depends on the SOURCE:
       - TEXT SPEC (dimensions written out, e.g. "SIZE: 110 x 75 x 155", "W110 x L75 x H155", "กว้าง 110 ยาว 75"): RESPECT THE STATED ORDER. Position 1 (or the W/กว้าง label) = width, position 2 (or L/ยาว/D label) = length — EVEN IF width > length. The customer states the order deliberately. DO NOT swap. e.g. "110 x 75 x 155" → {width:110, length:75, height:155}.
       - DIELINE IMAGE only (inferring from panel widths, NO W/L labels in the text): then length ≥ width is the convention (the larger horizontal panel = length). ONLY here, swap if you inferred width > length. This swap NEVER applies to dimensions stated in text.
...
function validateAndFix(data, hasImageInput) {
	...
	if (Array.isArray(data.components)) {
		data.components.forEach(comp => {
			...
			// Custom needs the FLAT-sheet size for the form's required กว้าง/ยาว, but
				...
				// จำไว้ว่ามิตินี้มาจากไดไลน์ไหม (ต้องเช็คก่อนลบ dieline_panels) → ใช้ตัดสินการสลับ length≥width
				const fromDieline = !!hasImageInput || (Array.isArray(comp.dieline_panels) && comp.dieline_panels.length > 0)
			...
			// Enforce length >= width ONLY for DIELINE-INFERRED dimensions (image input
			// or AI read panel widths) — the AI is unreliable there (D3D dieline came
			// back width=80/length=25; GEFEN 202/66). For a TEXT spec the customer states
			// W×L order deliberately (e.g. "110 x 75" = width 110, length 75) → RESPECT it,
			// do NOT swap even when width > length. Signal of dieline source: image input
			// or comp.dieline_panels present — flag จับไว้ก่อนลบ dieline_panels ด้านบน).
			if (fromDieline && comp.dimensions_mm && typeof comp.dimensions_mm === 'object') {
...
router.post('/parse-spec', upload.array('files', 10), async (req, res) => {
	try {
		...
		// length≥width swap applies to dieline/image inputs only — text keeps stated W×L order
		const hasImageInput = (files || []).some((f) => (f.mimetype || '').toLowerCase().startsWith('image/'))
		parsed = validateAndFix(parsed, hasImageInput)
