...
function validateAndFix(data) {
	...
	if (Array.isArray(data.components)) {
		data.components.forEach(comp => {
			...
			// Enforce length >= width (form convention: the longer horizontal
			// side is ยาว). The AI is told to swap when width > length but is
			// unreliable — the D3D dieline came back width=80/length=25. Do it
			// deterministically (GEFEN 202/66 -> 66/202, D3D 80/25 -> 25/80).
			if (comp.dimensions_mm && typeof comp.dimensions_mm === 'object') {
