...
- HOW MANY COMPONENTS (do not over-split): emit ONE component per physical piece that the SPEC SECTIONS actually describe — i.e. a piece that has its OWN size / paper / print (e.g. "PAPER - Box: A/C 350 / PAPER - Sleeve: A/C 300" = two components). NEVER create a component out of a PACKING line: "PACKING - Inner: kraftwrap 500 ชิ้น/ห่อ / Sleeve: kraftwrap 300 / Tray: kraftwrap 500" is PACKING data (how each piece is wrapped), NOT a component spec. If PACKING (or a remark) names extra pieces but SIZE/PAPER/PRINT give only ONE set of specs, emit exactly ONE component and list the extra piece names in "notes" so the estimator can add them. An EMPTY extra component is worse than none — every component's size/paper is REQUIRED in the form, so a blank one blocks the calculation and the estimator must delete it by hand.
